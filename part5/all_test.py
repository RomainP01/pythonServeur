...
class MyTestCase(unittest.TestCase):
    ...
    def test_word_should_be_in_db(self):
        ...
        cur = con.cursor()
        query = "CREATE TABLE partie5 (word text, following_word text)"
        con.execute(query)
    ...
    def test_get_a_dict_of_three_words(self):
        ...
        con.execute(query)
        con.commit()
        cur.execute("INSERT INTO partie5 VALUES('1','')")
        con.commit()
        cur.execute("INSERT INTO partie5 VALUES('2','')")
        con.commit()
        cur.execute("INSERT INTO partie5 VALUES('3','')")
        con.commit()
    ...
    def test_get_most_used_following_word(self):
        con = sqlite3.connect(":memory:")
        cur = con.cursor()
        query = "CREATE TABLE partie5 (word text, following_word text)"
        con.execute(query)
        con.commit()
        cur.execute("INSERT INTO partie5 VALUES('1','2')")
        con.commit()
        cur.execute("INSERT INTO partie5 VALUES('1','2')")
        con.commit()
        cur.execute("INSERT INTO partie5 VALUES('1','3')")
        con.commit()

        wfile = ""
        rapport = Rapport(con, wfile)
        self.assertEqual(rapport.get_first_value_from_dict(rapport.get_most_used_following_word("1")), "2")
        con.close()
